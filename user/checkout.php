...
try {
    // Fetch Cart
    $cart_sql = "SELECT c.product_id, c.quantity, p.product_name, p.price, p.image_url 
                 FROM cart c
    ...
    // Calculations
    $shipping = count($products) > 0 ? 59 : 0;
    $gst_amount = $total_amount * 0.10;
    $grand_total = $total_amount + $gst_amount + $shipping;
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        foreach ($products as $product) {
            ...
            $stmt->execute([
                ...
                ':total' => $grand_total, // Note: This stores the grand total of the whole cart for each item, might want to check DB schema intent, but keeping as per original logic
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .section-title { font-size: 1.25rem; font-weight: 600; color: var(--primary); margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem; }
        ...
        input {
            padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: var(--radius-md);
        ...
        .order-items { margin-bottom: 2rem; max-height: 300px; overflow-y: auto; padding-right: 0.5rem; }
        .order-item {
            display: flex; gap: 1rem; padding: 1rem 0; border-bottom: 1px solid #f1f5f9;
        }
        .item-img {
            width: 60px; height: 60px; object-fit: cover; border-radius: var(--radius-md);
            background: #f1f5f9;
        }
        .item-info { flex: 1; }
        .item-name { font-weight: 600; font-size: 0.95rem; color: var(--primary); margin-bottom: 0.25rem; }
        .item-meta { font-size: 0.85rem; color: var(--text-light); }
        .item-price { font-weight: 600; color: var(--text-main); }

        .summary-row { display: flex; justify-content: space-between; margin-bottom: 0.75rem; color: var(--text-main); font-size: 0.95rem; }
        .summary-row.total {
            border-top: 2px solid #f1f5f9; padding-top: 1rem; margin-top: 1rem;
        ...
        .place-order-btn {
            ...
            font-weight: 600; font-size: 1rem; cursor: pointer; margin-top: 1.5rem;
            transition: var(--transition);
        }
        .place-order-btn:hover { background: var(--accent-hover); transform: translateY(-2px); box-shadow: var(--shadow-md); }
        ...
        <form method="POST" action="">
            <div class="checkout-layout">
                ...
                <div class="form-card">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="first_name" required value="<?php echo htmlspecialchars($first_name_user); ?>">
                        </div>
                        <div class="form-group">
                            <label>Last Name</label>
                            <input type="text" name="last_name" required value="<?php echo htmlspecialchars($last_name_user); ?>">
                        </div>
                        <div class="form-group">
                ...
                <div class="summary-card">
                    ...
                    <div style="margin-top: 1rem; padding: 0.75rem; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: var(--radius-md); font-size: 0.9rem; color: #166534; display: flex; align-items: center; gap: 0.5rem;">
