...
// Fetch Orders
$query = "SELECT orders.id, orders.product_id, orders.quantity, orders.price, orders.status, orders.created_at, products.product_name, products.image_url 
          FROM orders 
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .order-body { display: flex; gap: 1.5rem; align-items: center; }
        .product-img {
            width: 80px; height: 80px; object-fit: cover; border-radius: var(--radius-md);
            background: #f1f5f9;
        }
        .order-info { flex: 1; }
        .product-name { font-weight: 600; color: var(--primary); margin-bottom: 0.25rem; }
        .order-meta { color: var(--text-light); font-size: 0.9rem; }
        .order-total { font-weight: 700; color: var(--accent); font-size: 1.1rem; margin-top: 0.5rem; }
        ...
        .btn-invoice { background: #f1f5f9; color: var(--primary); }
        ...
        .modal-title { font-size: 1.25rem; font-weight: 700; color: var(--primary); margin-bottom: 0.5rem; }
        .modal-text { color: var(--text-light); margin-bottom: 2rem; }
        .modal-actions { display: flex; gap: 1rem; }
        .modal-btn {
            flex: 1; padding: 0.75rem; border-radius: var(--radius-md); border: none;
            font-weight: 600; cursor: pointer; font-size: 1rem; transition: var(--transition);
        }
    ...
    <main class="container main-content">
        ...
        <?php if (count($orders) > 0): ?>
            <div class="orders-grid">
                <?php foreach ($orders as $order): 
                    ...
                    $gst = $totalPrice * 0.10;
                    $deliveryCharge = 59;
                    $finalTotal = $totalPrice + $gst + $deliveryCharge;
                    
                    $statusClass = 'status-processing';
                    $icon = 'fa-clock';
                    if(strtolower($order['status']) == 'delivered') { $statusClass = 'status-delivered'; $icon = 'fa-check-circle'; }
                    if(strtolower($order['status']) == 'cancelled') { $statusClass = 'status-cancelled'; $icon = 'fa-times-circle'; }
                        ...
                        <div class="btn-group">
                            <?php if ($order['status'] !== 'Cancelled'): ?>
                                <form method="POST" action="generate_pdf.php" target="_blank" style="display:inline;">
                                    ...
                                    <button type="submit" class="btn btn-invoice"><i class="fas fa-file-invoice"></i> Invoice</button>
                                </form>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <a href="index.php" class="btn" style="background: var(--accent); color: white; padding: 0.75rem 2rem;">Start Shopping</a>
            </div>
        <?php endif; ?>
    </main>
